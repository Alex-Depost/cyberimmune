...
def select_auto_and_prepayment(name, experience, car_model, tariff):
    response = requests.post(f'{MANAGMENT_URL}/select/car/{car_model}', json={'client_name': name, 'experience': experience, 'tariff': tariff})
    if response.status_code == 200:
...
def start_travel(car_model):
    response = requests.post(f'{CARS_URL}/car/start/{car_model}')
    if response.status_code == 200:
        print(response.json()['message'])
        return response.json()
    else:
...
def stop_travel(car_model):
    response = requests.post(f'{CARS_URL}/car/stop/{car_model}')
    if response.status_code == 200:
        print(response.json()['message'])
        return response.json()
    else:
